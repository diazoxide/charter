...
- **Read by:** `workspace.read_manifest` (`charter/workspace.py:1500`), `manifest_owner`
  ...
  the executor's gate for a view on that workspace's strip (charter-app#280); its
  `settings.theme` by `crates/charter-core/src/extension/project/theme.rs` (`Said::read_in`,
  `colour_of`) for the Workspace settings tab, the theme the window draws while the workspace is
  in front, and every workspace tab's colour (charter-app#281).
...
| any other key in `settings`, or `settings` that is not an object | — | Refused by the save, and ignored by the reader: a workspace's settings hold `extensions` and `theme`. Harness plugins are a later table (charter-app#282). | `crates/charter-core/src/settings/workspace.rs` `refusals` |
