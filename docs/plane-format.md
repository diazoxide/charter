...
| `[theme].use` | str | optional | **charter-app only** (charter-app#273, ADR 0048). This machine's pick of the project's theme, over `charter.toml`'s. Same values and rules as there; falls through to `charter.toml`'s pick when it is none of the shapes. | stable | `crates/charter-core/src/extension/project/theme.rs` `resolve` |
...
- **Read by:** `workspace.read_manifest` (`charter/workspace.py:1500`), `manifest_owner`
  ...
  the executor's gate for a view on that workspace's strip (charter-app#280); and by
  `crates/charter-core/src/harness_plugin.rs` (`Choices::read_in`) for the Workspace settings
  tab and for every chat started in the workspace (charter-app#282).
...
| any other key in `settings`, or `settings` that is not an object | — | Refused by the save, and ignored by the reader: a workspace's settings hold `extensions` and `harness_plugins` and nothing else yet. A theme is a later table (charter-app#281). | `crates/charter-core/src/settings/workspace.rs` `refusals` |
