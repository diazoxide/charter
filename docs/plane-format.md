...
| `relaunch_after_update` | bool | default `false`; written only when `true` | the quit that wrote this restarted charter to install an update (charter-app#251), so the launch after it says why it is asking and offers "Reopen all" first. Every later write is an ordinary one and drops it, so it lasts one launch |
