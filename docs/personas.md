...
**The generated file is the one the sub-agent actually reads**; `persona.md` is the one a
human edits. A stale copy therefore never fails — it silently dispatches an older charter —
so keep the two committed together. `charter persona lint` names any persona whose sub-agent
has fallen behind, and charter's own suite fails on it (`--only stale` narrows `lint` to that
one finding, which is what a CI step or a news `check:` wants).

Both halves are also regenerated by charter itself, not only by an edit to `persona.md`: a
correction to the wording charter injects moves every generated file, and that difference is
invisible in a diff between the two committed ones.

Run inside a **linked worktree** of the plane, `sync-agents` regenerates that worktree's
`.claude/agents/` from that worktree's `personas/`, and prints where it wrote. It never edits
the main clone — see [Working inside a worktree](control-plane.md#working-inside-a-worktree)
for which operations follow the plane and which follow the tree.
...
The last line is for a persona whose `persona.md` is there but does not load, for example
because it is not UTF-8, cannot be read, or links out of the plane. It has no create hint,
because `persona create` refuses a name whose file is there, and `persona lint devops` says
why the file does not load (#1061). Before, that persona was reported as absent with the
create hint, and a file that could not be read or decoded crashed the command, `persona
lint` included.
