...
| `tools` | **Binaries** auto-approved (no permission prompt) while this persona is active — the `PreToolUse` tool-gate reads this. The unit is the binary and every argument rides along, so `tools: gh` is `gh` doing anything, `gh repo delete` included. Four things are never smoothed however they are declared: destructive subcommands (`kubectl delete`, `charter secret`, `git clean`, …), interpreters and wrappers whose *argument* is the real command (`bash`, `python3`, `env`, `xargs`, `npx`, …), any command whose arguments reach a vault or charter's own state — decided by resolving each argument to the file it opens, not by matching its text, so quoting, escaping, globbing, a symlink, the bare directory name and a directory that contains it are one answer (see [hooks.md](hooks.md)) — and anything added to this line **after the session started**, which takes effect in the next session. |
