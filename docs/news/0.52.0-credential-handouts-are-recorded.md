...
Before a line is written, charter walks the record it is about to write and rewrites any
resolved value it finds, at any depth — so the field somebody adds next year is covered by
the rule that is already there. This is about the trace file charter writes, not about what
a command charter runs does with the value it was handed. The rest of `argv` is deliberately not recorded: charter never substitutes a
