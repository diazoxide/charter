...
reason. With `--from-file` the value comes off disk and this line is all charter prints, so
the exact length used to be the one thing the line disclosed to whoever read it.
