...
- **A path charter did not generate is never touched.** The `.charter-generated` sidecar
  ...
  — it is not hidden either. In this release a path you took over stopped being hidden on the
  next wire; since #942 it stays hidden while it is there, is never overwritten, and
  `charter doctor` names it `foreign`.
