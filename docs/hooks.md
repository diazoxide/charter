...
The host reads `permissions` from the session's own directory and does not walk up, and a
framed chat stands in `workspaces/<name>/`. So charter carries the **restrictive** half —
`ask` and `deny`, never `allow` — into the file it generates for each workspace and each
clone inside one, and `charter guard ask` refreshes those as it writes. Without that the
rule was written, reported, and not in force where the guarded command gets typed (#942).
