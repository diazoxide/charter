...
- **`charter save --pull` brings in what the remote has before it saves.** A chat the app did
  ...
  tree has conflicts, the command stops and saves nothing.
...
- **The dispatch log, the session trace and a memory index refuse to write through a link.**
  Each was checked to lie inside the plane and then opened in a separate step, so a link put
  in its place between the two could redirect the write to another file in the plane. They
  now open the file without following a link, and refuse it when it is one.
