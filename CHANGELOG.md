...
- A handoff can ask for an answer. With `charter handoff --report`, the new chat is told to
  ...
  typed into it. The report goes only to the chat that asked, once per handoff until you give the
  new chat another turn; if that chat has closed, the next chat in its workspace learns it when
