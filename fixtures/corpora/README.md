...
| `shellseg-oracle.jsonl` | Command lines the Python charter's `hooks.py` docstrings name as bypasses that SHIPPED, each with what the frozen Python answers for all 48 functions M3.1 has ported so far — the shell reader, the heredoc layout, the wrapper split and the leak guard. Recorded, not written: `tests/differential/shellseg.py --record` produced it and `--check` fails if it stops matching the oracle. Replayed offline by `crates/charter-core/tests/the_shell_is_read_the_way_python_reads_it.rs` and `…/the_leak_guard_answers_what_the_python_answers.rs`, so the plain `cargo test` job holds the line with no Python present. Every filesystem answer in it is a path **relative to the fixture plane the harness builds**, never an absolute one, so the file is the same on every machine; the probe INPUTS ride along in the `pr` key, which is what lets the replay tests hold no copy of the harness's tables. |
