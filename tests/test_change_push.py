...
#: Author identity only. The config that used to matter here — `commit.gpgsign` and
#: 1Password's `op-ssh-sign`, which parked every fixture commit on a biometric prompt — is
#: neutered for the whole suite by `tests._gitguard` now (#641), so this no longer carries
#: `GIT_CONFIG_GLOBAL`: one answer to "what git config does a child read", in one place.
