...
def _unquoted(text: str) -> str:
    """Quoted spans resolved, per the rule above :data:`_QUOTED`.

    A span that is a claim on its own is blanked — the retraction case, unchanged — and so
    now is a span that is merely CLAUSE-SIZED, which is the correction.

    Round four blanked only the first, and kept the second's words in the sentence. That is
    how `docs/news/0.52.0-the-vault-says-what-it-guards.md` came to be flagged on its own
    retraction: the frame supplied *a secret is*, the quotation supplied *cannot leak it
    into the transcript*, neither half is a claim on its own, and the sentence the detector
    then judged appears on no page in this repo. It is `spliced_words`' shape one level up
    — there a word the page shows and the file does not spell, here a claim the detector
    reads and nobody wrote — and it is answered the same way: refuse the splice, rather
    than learn to recognise the frame. A list of reporting verbs (*told*, *said*,
    *claimed*, *used to read*) was the alternative, and this file has now watched five
    lists be one entry short.

    Blanking rather than fencing, because a quoted sentence carries its own full stop:
    fencing it with a clause boundary bounded the side the frame is on and left the other
    side in the NEXT sentence, which is the same splice with an extra step.
    """
    def keep_or_blank(m: re.Match) -> str:
        inner = m.group()[1:-1]
        return (" " * len(m.group())
                if _is_a_claim(inner) or _is_clause_sized(inner) else inner)
    return _QUOTED.sub(keep_or_blank, text)


def _is_clause_sized(span: str) -> bool:
    """Could someone have written *span* as a clause, rather than slotted it into one?

    This is the line between the two things quotation marks do, and it is drawn on the
    span's own shape rather than on the words around it. An EVASION quotes a token —
    *the model never sees the "value" in your transcript* — and the token is a word of the
    sentence whoever wrote it meant; fencing it off would hand the evader a clause break,
    and `TestTheDetectorFires` proves that by failing when it does. A REPORT quotes
    something that stands on its own: it carries its own internal clause break or its own
    terminal punctuation, because it was a clause before it was quoted.

    A span that is clause-sized is fenced (U+201F, in both clause walks) rather than
    deleted, so its words still count where they are and no clause reaches across the
    quotation mark. A span that is not is left exactly as it was.
    """
    return bool(_CLAUSE_END.search(span)) or span.rstrip().endswith((".", "!", "?"))


#: One sentence. A terminator is `.`/`!`/`?` that is NOT glued to the next word, which is
#: a property of tokens rather than a list of abbreviations — the round before this one
#: split on every period there was, so `skills/secrets/SKILL.md told the agent that …`
#: became a sentence beginning `md told …` with its subject amputated, and
#: `charter/frame/layout.py:127` and three 0.52.0 news entries were each flagged on a
#: fragment nobody wrote. `0.52.0`, `open(2).`, `str.replace` and every `<name>.md` in
#: these pages are the same one shape. A list of abbreviations would be the next round's
#: bypass and would not have covered any of these; "the period is inside a word" covers
#: all of them and needs no entries.
_SENTENCE = re.compile(r"(?:[^.!?]|[.!?](?=[^\W_]))+[.!?]*")
...
def _is_a_claim(text: str) -> bool:
    ...
    return any(unqualified_promise(s) or total_redaction(s)
               for s in _SENTENCE.findall(text) if s.strip())
...
def _split(block: str) -> list[tuple[str, str]]:
    ...
    spans = [m for m in _SENTENCE.finditer(text) if m.group().strip()]
...
def _promise_fails(sentence: str, head: str, clause: str, m: re.Match,
    ...
    if not _SUBJECT.search(subject):
        return False
    if _is_an_instruction(sentence, m):
        return False
...
def _is_an_instruction(sentence: str, m: re.Match) -> bool:
    """Is this a rule addressed to the reader rather than a promise made to them?

    **A promise names who keeps it; an imperative names nobody, because the reader is the
    one being told.** `skills/secrets/SKILL.md`'s hard rules are the whole reason this
    exists — *"Never read a vault file, by any name, spelling, program, or recursive walk
    that happens to include it."* is the strongest sentence in that file and it is not a
    claim about where a value ends up; it is an instruction to the model, and flagging it
    would push the file towards saying something weaker to the one reader that cannot go
    and check. Round four had no such distinction: an empty head is read as "the promise
    is about the value itself", which is right for *"the value never reaches the
    transcript"* and wrong for *"Never read it"*.

    Narrow on two axes, and the anti-vacuity fixtures decide both.

    *Position*: a CLAUSE-LOCAL absolute word standing as the sentence's very first word.
    A sentence-scope adverbial does not qualify — *"In every case the value is injected
    into the subprocess and redacted from its output"* also begins at offset zero and is
    one of the six sentences that actually shipped.

    *Part of speech*: the match must be a bare ADVERB. This is two entries and it is not a
    list one entry short, because it is the complement of a grammatical fact: every other
    alternative in `_ABSOLUTE_LOCAL` that can begin a sentence introduces a SUBJECT —
    `not one` and `no <noun>` are determiners, `nobody` is a pronoun — and a subject is
    precisely what an imperative does not have. Widening this to positional-only was
    written and immediately turned three fixtures green that must be red: *"Not one secret
    ever reaches the transcript."*, *"Nobody watching this conversation ever sees the
    value."* and the README's *"no step here ever put the value in a context window"*. All
    three are promises with a subject, spelled to start where an imperative starts.
    """
    return (m.re is _ABSOLUTE_LOCAL
            and m.start() == 0 and not sentence[:m.start()].strip()
            and m.group().strip().lower() in ("never", "always"))
...
class TestTheUnitIsWhatSomebodyWrote(unittest.TestCase):
    """Three ways this file judged a sentence that appears on no page in this repo.

    All three arrived the same way — the corpus grew (0.52.0's news entries, and the two
    security branches merged beside this one) and the detector met prose it had never been
    run over. None is a new idea; each is the detector splicing or amputating text and then
    judging the result, which is `spliced_words`' own finding turned on the detector rather
    than on the page.

    Each test below is the reproduction. Without them the three fixes are invisible: every
    one of them was GREEN under mutation before this class existed, because the assertions
    that catch them live in files nobody had run the detector over.
    """

    def test_a_period_inside_a_word_does_not_end_a_sentence(self):
        """`skills/secrets/SKILL.md told the agent that a secret is …` became a sentence
        beginning `md told the agent …` — subject amputated, and then judged. Four sites
        were flagged on fragments nobody wrote: two `charter/frame/layout.py` docstrings
        and two 0.52.0 news entries. Every spelling here is real text from this repo.
        """
        for text, want in (
                ("skills/secrets/SKILL.md told the agent that a secret is masked.", 1),
                ("The cost is stated in docs/frame.md beside the other named costs.", 1),
                ("Redaction is str.replace over the bytes that came back.", 1),
                ("0.52.0 shipped it.", 1),
                ("The mode argument to open(2) applies only when it creates the inode.", 1),
                ("One. Two. Three.", 3),
                ("Denied. charter refuses it.", 2)):
            with self.subTest(text=text[:40]):
                got = [x for x in _SENTENCE.findall(text) if x.strip()]
                self.assertEqual(len(got), want, got)

    def test_a_promise_is_never_assembled_across_a_quotation_mark(self):
        """A frame supplying the subject and a quotation supplying the predicate is a
        sentence on no page. `docs/news/0.52.0-the-vault-says-what-it-guards.md` retracts
        SKILL.md's old wording and was flagged on the retraction: neither half is a claim
        alone, and the concatenation is.
        """
        report = ('skills/secrets/SKILL.md told the agent that a secret is "redacted from '
                  'its output, so a command that echoes it still cannot leak it into the '
                  'transcript." Redaction is str.replace over the bytes that came back.')
        self.assertEqual(flagged(report), [])

    def test_but_quoting_a_TOKEN_still_does_not_launder_a_promise(self):
        """The counter-control, and the reason the rule is about the SPAN's shape rather
        than about the frame around it. A quoted word is a word of the sentence whoever
        wrote it meant; fencing or blanking it would hand an evader a free clause break.
        These three are in `_BYPASSED` and must stay red.
        """
        for s in ('The model never sees the "value" in your transcript.',
                  'charter never prints the value into the "conversation".',
                  'No "secret" ever reaches the transcript.'):
            with self.subTest(sentence=s[:40]):
                # `flagged`, not `unqualified_promise`: quotation is resolved in `_split`,
                # so a test that called the detector directly would never exercise the
                # rule it names.
                self.assertNotEqual(flagged(s), [], s)

    def test_a_rule_addressed_to_the_reader_is_not_a_promise(self):
        """`skills/secrets/SKILL.md`'s hard rules are instructions to the model, and the
        strongest of them was flagged. Pushing that file towards a weaker sentence, to
        please a detector, is the exact defect #444 is about — aimed at the one reader who
        cannot go and check.
        """
        for s in ("Never read a vault file, by any name, spelling, program, or recursive "
                  "walk that happens to include it.",
                  "Never echo a secret into the transcript.",
                  "Always delete the file when the tool is done."):
            with self.subTest(sentence=s[:40]):
                self.assertFalse(unqualified_promise(s), s)

    def test_and_a_subject_that_merely_starts_the_sentence_is_still_a_subject(self):
        """The counter-control for the one above, and it caught the first version of it.
        `not one`, `no <noun>` and `nobody` also stand at offset zero — they are
        determiners and a pronoun, not adverbs, and each introduces the subject an
        imperative does not have. A positional-only exemption turned all three green.
        """
        for s in ("Not one secret ever reaches the transcript.",
                  "Nobody watching this conversation ever sees the value.",
                  "no step here ever put the value in a context window"):
            with self.subTest(sentence=s[:40]):
                self.assertTrue(unqualified_promise(s), s)
