...
running that file, not by hand.

**Re-record it on every rebase, and that is not housekeeping.** A pinned floor is a floor
against the ONE commit it was taken from. This corpus was first recorded against 060d584;
by the time the branch merged, `main` had closed #462 and denied 798 more of these inputs
than the recording knew about. The test would have passed on all 1,646 old rows while
saying nothing at all about the 798 — a green floor under a main nobody merges into, which
is the runtime-baseline defect with a fixture in front of it. Regenerate by loading main's
...
class DeniedByMainStaysDenied(PersonaIso):
    ...
    def test_the_corpus_is_actually_a_corpus(self):
        ...
        rows = _corpus()
        # 2,444 rows at the 18a8afc recording. The floor is well under it so an honest
        # re-record can shrink a little, and well over the 1,646 of the previous one so a
        # corpus silently reverted to the stale recording fails here rather than passing
        # every assertion below against a main that is two releases old.
        self.assertGreater(len(rows), 2000, "corpus too small to be a floor")
