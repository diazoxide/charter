# Memory Index — statusline

One line per memory; each links a file holding a single durable fact.
Written by the persona as it learns; committed and shared.
- [Alignment breaks on width that differs BETWEEN rows, not on width itself](alignment-breaks-on-width-that-differs-between-r.md)
- [Use East-Asian Neutral glyphs in width-critical cells: U+25AA ▪, U+25B8](use-east-asian-neutral-glyphs-in-width-critical-.md)
- [Claude Code's footer collapses a BLANK row to column 0, so a row that mu](claude-code-s-footer-collapses-a-blank-row-to-co.md)
- [_TREE_WT (╰─) must stay textually distinct from _TREE_END (└─): render's](tree-wt-must-stay-textually-distinct-from-tree-e.md)
- [The status line renders on EVERY turn and is fast, but it is NOT subproc](the-status-line-renders-on-every-turn-and-is-fas.md)
- [Regenerating docs/assets captures: the render's ACTIVE PERSONA is as ses](regenerating-docs-assets-captures-the-render-s-a.md)
- [Glyph vocabulary already spoken for on a persona CHIP in charter/statusl](glyph-vocabulary-already-spoken-for-on-a-persona.md)
- [A source edit that RESTORES a file to its exact previous size within the](a-source-edit-that-restores-a-file-to-its-exact-.md)
- [One glyph, one meaning — and the tiebreak when two surfaces want the sam](one-glyph-one-meaning-and-the-tiebreak-when-two-.md)
- [A statusline surface that DELETES the record it draws from turns the mos](a-statusline-surface-that-deletes-the-record-it-.md)
- [Regenerating docs/assets captures: a statusline feature that renders onl](regenerating-docs-assets-captures-a-statusline-f.md)
- [Regenerating charter's captured README assets (docs/assets/): the pipeli](regenerating-charter-s-captured-readme-assets-do.md)
- [Regenerating docs/assets captures on a machine with git commit signing:](regenerating-docs-assets-captures-on-a-machine-w.md)
- [docs/assets captures, proving a re-run when output is byte-identical: on](docs-assets-captures-proving-a-re-run-when-outpu.md)
- [Never 'git checkout -- <file>' to undo a temporary mutation-check edit:](never-git-checkout-file-to-undo-a-temporary-muta.md)
- [Bar measurements written as PANE WIDTHS go stale the moment the lead cha](bar-measurements-written-as-pane-widths-go-stale.md)
- [On a strip whose clicks resolve by COLUMN, any new per-tab field must be](on-a-strip-whose-clicks-resolve-by-column-any-ne.md)
- [tmux's `resize-pane -Z` is a TOGGLE — no version has an unzoom flag, and](tmux-s-resize-pane-z-is-a-toggle-no-version-has-.md)
- [The outer terminal's mouse mode follows tmux's ACTIVE pane, not the zoom](the-outer-terminal-s-mouse-mode-follows-tmux-s-a.md)
- [A pty measurement that attaches a real tmux client fails silently on CI:](a-pty-measurement-that-attaches-a-real-tmux-clie.md)
- [Charter's strips refuse non-ASCII glyphs as a property of the ROW, not o](charter-s-strips-refuse-non-ascii-glyphs-as-a-pr.md)
- [Confirmation surfaces size themselves to their blast radius (#927): chat](confirmation-surfaces-size-themselves-to-their-b.md)
- [A charter panel repaints ONLY when its own state.version(fid) bumps (pan](a-charter-panel-repaints-only-when-its-own-state.md)
- [A charter frame is laid out on ONE chat at a time: cmd_chat step 2 is _a](a-charter-frame-is-laid-out-on-one-chat-at-a-tim.md)
- [A child charter a test starts (tmux panel, detached frame-gather, 'chart](a-child-charter-a-test-starts-tmux-panel-detache.md)
- [capture-frame.sh private tmux (2026-09-11, branch readme-shows-the-frame](capture-frame-sh-private-tmux-2026-09-11-branch-.md)
- [SVG captures of the chat strip: slots.TAB_SPINNER cycles ✢✶✻✶, and ✻ ren](svg-captures-of-the-chat-strip-slots-tab-spinner.md)
- [The frame repo table has NO presence column by design (frame/slots._tabl](the-frame-repo-table-has-no-presence-column-by-d.md)
- [docs/assets/demo-plane.sh used to run charter init with the caller PATH](docs-assets-demo-plane-sh-used-to-run-charter-in.md)
- [Operator config fingerprints during a capture are not proof on their own](operator-config-fingerprints-during-a-capture-ar.md)
- [The frame strip's arrived mark (charter/frame/slots.py _ARRIVED_MARK) is](the-frame-strip-s-arrived-mark-charter-frame-slo.md)
- [tests/_statedirscan.py classifies a function as returning a STATE PATH f](tests-statedirscan-py-classifies-a-function-as-r.md)
