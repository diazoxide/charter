...
// (ADR 0015). It is not silent about it — `charter doctor` compares this file to the one
// charter generates, byte for byte, and names it as one charter cannot vouch for, because
// the guards below are the only thing standing between a tool call and the vault.
...
def shim_is_charters(tree: Path) -> bool:
    ...
    """
    try:
        return (Path(tree) / SHIM_PATH).read_text() == SHIM
...
class OpenCodeHarness(Harness):
    ...
    def stale_wiring(self) -> str:
        """What the installed plugin IS, when it is not the one this charter generates.

        A phrase the caller drops into "its plugin is …", so every answer has to read as
        one. ``""`` when the file is byte-for-byte charter's own — the only state charter
        can vouch for — or absent, which is `harness`'s other sentence and two rows saying
        the plane is unwired teaches people to skim.

        The empty answer is decided by CONTENT (:func:`shim_is_charters`), not by the
        stamp. A stamp is a name a file carries, and "the running version" was reported for
        a body with every guard cut out of it, because line 1 is the one line an edit
        leaves alone. That is #433's shape — wiring that looks present and does not run —
        moved from the routing table to the file the routing table lives in.
        """
        g = global_dir()
        if not (g / SHIM_PATH).is_file():
            return ""
        if shim_is_charters(g):
            return ""
        got = shim_version(g)
        if got is None:
            return "unstamped"
        if got == __version__:
            return f"stamped {got} but changed since charter wrote it"
        return got
    ...
    def upgrade(self, root: Path) -> tuple[str, str]:
        ...
        stamp" when it plainly does sends them looking for the wrong thing.
        """
        g = global_dir()
        got = refresh_shim(g)
        if got == "current":
        ...
        if got == "edited":
            return "manual", (f"{g / SHIM_PATH} is stamped {__version__} but is not what "
                              f"charter generates, so charter will not overwrite it — "
                              f"diff it, then move it aside and run `charter reinit`")
