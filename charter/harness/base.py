...
class Harness:
    ...
    def refresh_tree(self, tree: Path) -> list[tuple[str, str]]:
        """Bring *tree*'s wiring up to this charter, replacing what charter itself wrote.

        Separate from :meth:`wire_tree`, which may never repair because it cannot ask
        whether a file is charter's. This can, and still refuses the moment the answer is
        no — so `reinit` fixes a stale generated shim and leaves an edited one alone.
        """
        return self.wire_tree(tree)

    def wire_tree_stale(self, tree: Path) -> str:
        """Which charter wrote this tree's wiring, when that is not the running one.

        Returns a version string (or ``"unstamped"``) to report, or ``""`` when the tree
        is current or this harness writes nothing per tree. Existence is not the question
        `doctor` should be asking on its own: a plugin generated by 0.40.0 threw on every
        tool call and failed open, and it is still a file sitting exactly where a wired
        one belongs.
        """
        return ""
