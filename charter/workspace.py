...
def wire_harnesses(name: str) -> list[tuple[str, str]]:
    ...
    ``"blocked"`` · ``"withheld"`` · ``"harness-behind"`` · ``"unreadable"`` (a co-written
    file charter cannot read, never folded into ``"foreign"``).
...
def _withdraw(base: Path, want_all: dict[str, str], marker: dict) -> list[tuple[str, str]]:
    ...
    **Which files** is :func:`_unwanted`'s answer, asked rather than re-derived: content still
    ...
    for rel in [r for r in marker if not os.path.lexists(base / r)]:
        del marker[rel]
    return rows
...
def _materialise(base: Path, want_all: dict[str, str],
    ...
    *withhold* names wanted paths that must not be written this time. :func:`wire_guest`
    ...
    for rel, status in _layer_status(base, want_all, marker):
        if status == "unreadable" and rel in cowritten:
            # Not `foreign` (#942, review round 2). A file the harness also writes into may
            # hold its approvals, and every `foreign` sentence advises removing the file.
            # Charter cannot see what is in it, so it says exactly that and leaves it as it is.
            rows.append((rel, "unreadable"))
            continue
...
def _live_trees(tree: Path, exclude: Path) -> list[Path] | None:
    """Every working tree whose git reads *exclude* — *tree* among them — or ``None`` when
    git cannot say.
    ...
    **Git is the registry**, as it is for `worktree.py`: ``git worktree list --porcelain``,
    ...
    hand does not. A tree git still lists after its directory went reads as holding nothing
    and needing nothing, which is what it needs.
    ...
    *tree* — and a launch wires every checkout in its workspace, at ~7 ms a spawn (measured).
    ...
    if proc.returncode != 0:
        return None
    return [Path(w["path"]) for w in _worktree.parse_porcelain(proc.stdout)]


def _shared_rels(tree: Path, rels: list[str], text: str, exclude: Path) -> list[str]:
    ...
    **When git cannot say which trees there are, nothing leaves the block** — hiding a path
    a little longer than it needs is the direction that costs nobody anything.
    """
    lines = text.splitlines()
    span = _block_span(lines)
    current = ({line[1:] for line in lines[span[0] + 1:span[1] + 1] if line.startswith("/")}
               if span else set())
    need = set(rels)
    trees = _live_trees(tree, exclude)
    if trees is None:
        need |= current
        trees = [tree]
    for t in trees:
        need.update(_charter_owned(t, _read_marker_at(t)))
    need.update(rel for rel in current & _cowritten()
                if any(os.path.lexists(t / rel) for t in trees))
    ordered = sorted(need - {GENERATED_MARKER})
    return ordered + [GENERATED_MARKER] if GENERATED_MARKER in need else ordered


def _exclude_status(tree: Path, rels: list[str]) -> str:
    """``ok`` · ``missing`` · ``stale`` · ``unreadable`` for the block in *tree*'s exclude.

    Compared against :func:`_shared_rels`, the block :func:`_register_excludes` writes. Against
    *rels* alone, a block that rightly holds a sibling worktree's line reads `stale` for ever.
    """
    ...
    try:
        ...
        return "unreadable"
    if _replace_block(text, _exclude_block(_shared_rels(tree, rels, text, p))) == text:
        return "ok"
...
def _register_excludes(tree: Path, rels: list[str]) -> str:
    ...
    *rels* is what *tree* needs; the block written is :func:`_shared_rels`' — never one
    tree's list alone, for the sibling worktrees reading the same file."""
    p = git_exclude_file(tree)
    if p is None:
        return "blocked"
    try:
        ...
        return "blocked"
    new = _replace_block(text, _exclude_block(_shared_rels(tree, rels, text, p)))
    if new == text:
        return "present"
...
def unwire_guest(tree: Path) -> list[str]:
    ...
    **A co-written file that stays keeps its line** (#942). Once the harness has saved its
    ...
    try:
        ...
        pass
    # Nothing of this tree's own is named. A co-written file that stays keeps its line by
    # existing, and every other tree keeps what it owns — both `_shared_rels`', so the
    # removal and the wire cannot disagree about either. The first version named the kept
    # files itself, which answered for this tree and not for the trees beside it.
