...
// PROOF ONLY, never merge — what the worktree listing costs a launch.

#[test]
fn proof_only_what_the_worktree_listing_costs() {
    use std::time::Instant;

    let f = layered_plane("thing");
    let ws = f.workspace();

    // (a) A workspace whose checkouts have NO linked worktrees: the common git directory has
    // no `worktrees/`, so neither the pieces nor the exclude block spawns git at all.
    let bare = {
        let start = Instant::now();
        for _ in 0..20 {
            let _ = charter_core::wslayer::guest_trees(&ws);
        }
        start.elapsed() / 20
    };

    // (b) The same once the repository HAS linked worktrees: one `git worktree list` for the
    // whole call, shared by the pieces and the exclude block.
    cut(&f, "one");
    cut(&f, "two");
    cut(&f, "three");
    let listed = {
    ...
    // (c) And a whole wire of that workspace, which is what a launch pays.
    let wire = {
        let start = Instant::now();
        for _ in 0..10 {
            let _ = charter_core::wslayer::wire(&f.plane, &ws);
        }
        start.elapsed() / 10
    };

    panic!("MEASURED no-worktrees {bare:?} | three-worktrees {listed:?} | whole wire {wire:?}");
}
