...
it to 0600 once the write returned. The comment above that line claimed the plaintext was
"never briefly world-readable." It was wrong about its own mechanism: the mode argument to
`open(2)` applies **only when the call creates the inode**. For a vault someone had
...
[#470](https://github.com/diazoxide/charter-plane/issues/470) rather than fixed here, because
...
green line was [#471](https://github.com/diazoxide/charter-plane/issues/471), and it landed in
