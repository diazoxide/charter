...
**Affected: every release with the vault guard, through 0.61.0.** The Bash vault guard refuses
...
Stdin stays data: `-F -` (and `gh api`'s `@-`) is the body a heredoc feeds gh, which this
release reads as text. An ordinary body file (`-F notes.md`) is untouched, and so is
...
([#1086](https://github.com/diazoxide/charter-plane/issues/1086)).
