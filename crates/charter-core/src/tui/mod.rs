...
//!
//! The tables are **generated from CPython's own `unicodedata`**
//! (`tools/gen-unicode-tables.py` → [`tables`]) rather than taken from a crate:
//! `unicode-width` deviates from
...
//!
//! # [`tables`] is the crate's one generated Unicode table file
//!
//! It outgrew this module's own question. [`crate::shown`] and [`crate::pyrepr`] each used to
//! carry a hand-written table for "which characters have no glyph", derived from the same
//! `unicodedata` by hand at different times — and the two had already drifted apart by nine
//! ranges, so one of them escaped U+0890 and the other printed it. They are generated by the
//! same script now, from the same CPython, with the Unicode version named once at the top of
//! the file. The tables live here rather than somewhere neutral because there is nowhere
//! neutral that is not a fourth place to look: this is where the generator and the search
//! already were, and moving them would cost a reader the one file that answers
//! "where does charter's Unicode come from".
...
/// Is `ch` in one of `table`'s inclusive ranges?
///
/// `pub(crate)` because [`tables`] is now the crate's ONE generated Unicode table file and
/// the other two questions in it are asked from outside this module: [`crate::shown`] asks
/// [`tables::INVISIBLE`] ("has this character no glyph, so printing it could forge a line?")
/// and [`crate::pyrepr`] asks [`tables::NONPRINTABLE`] ("would `repr()` escape it?"). Both
/// used to carry a hand-written table of their own, and the two disagreed — `shown` escaped
/// U+0890 and `pyrepr` printed it — which is the failure a generated table exists to stop.
/// One search over one shape, so a table cannot grow a second lookup with its own opinion.
///
/// A binary search: [`tables::NONPRINTABLE`] carries every unassigned codepoint and is
/// therefore hundreds of ranges long, and it is asked once per character of every value
/// charter quotes. The tables are sorted and disjoint by construction — the generator walks
/// the codepoints in order and closes a range the moment the predicate stops holding — which
/// is the whole of what the search needs.
pub(crate) fn in_table(ch: char, table: &[(u32, u32)]) -> bool {
    let cp = ch as u32;
    table
