...
pub fn install(
    ...
    // The generator writes where it runs: the plane's own `.claude/skills/`, and a committed
    // link there that leaves the plane is not somewhere charter points a generator.
    if !crate::contain::within_plane(root, &skill_dir) {
        say(Say::Fail(format!(
            "{SKILL_DIR} resolves out of the plane — charter will not run a generator that \
             writes through it."
        )));
        return 1;
    }
    crate::fence::hold(crate::fence::Act::Generate, root);
...
mod tests {
    ...
    #[cfg(unix)]
    #[test]
    fn a_skills_folder_linked_out_of_the_plane_is_never_generated_into() {
        let dir = plane();
