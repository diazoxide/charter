 ...
 *
 *  - `unwired` is the ordinary state of a worktree charter cut today. The harness layer is
 *    not ported yet, so a chat there runs with **none of the plane's ask/deny rules**, none
 *    of its persona's agents, and no `$CHARTER_HARNESS`. That is a guard hole, and a row
 *    that stayed silent about it would let the operator believe guards were on.
    ...
    <span className="worktree">
      ...
      {!worktree.wired && !worktree.stale && (
        <span
          ...
          title="No charter layer in this worktree: no persona agents, no ask/deny rules, no $CHARTER_HARNESS. A harness started here runs without them."
        >
          unwired
        </span>
      )}
