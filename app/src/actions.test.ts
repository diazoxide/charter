...
describe("the palette at fifty chats", () => {
  ...
  it("still offers one row per chat and per workspace, browsable with nothing typed", () => {
    ...
    // 346 rows: 50 chats four times over, 6 workspaces THREE times, 50 pieces TWICE, 8
    // personas, 2 in the queue TWICE (show it, and ignore it — charter-app#248), and the
    // verbs. It was 344 before the vault picker and New vault… (charter-app#235; this plane
    // has no vaults, so no `vault.open:` rows), 118 before the pins, 174 before
    ...
    expect(offers).toHaveLength(346);
  });
