...
export function declareAProfile(plane: string, program: string, kind = "claude"): void {
  ...
  writeFileSync(
    ...
    [
      ...
      "  exit 0",
      "fi",
      "# What the chat's environment says about the harness's own footer (charter ADR 0029),",
      "# written down where a scenario can read it. Claude Code's footer command inherits",
      "# this environment; the fake harness draws no footer, so this file is how a scenario",
      "# sees what a real harness would have been started with. The chat's own number keys",
      "# it, so two chats in one run never write over each other.",
      `if [ -n "\${CHARTER_ROOT:-}" ] && [ -n "\${CHARTER_CHAT:-}" ]; then`,
      '  mkdir -p "$CHARTER_ROOT/.charter/scenario"',
      "  printf '%s' \"${CHARTER_HARNESS_FOOTER:-}\" \\",
