...
pub fn preview(root: &Path, name: &str) -> Result<LivePreview, String> {
    ...
    Ok(LivePreview {
        ...
        remote: planegit::origin_https(root),
...
pub fn switch(root: &Path, name: &str, live: bool) -> Result<Vec<String>, String> {
    ...
    // Saved at once, whatever auto-save is set to: this is the operator publishing, or
    // withdrawing, on purpose. A save the plane's mode stops short of is the mode's to say.
    match crate::saving::save_as(root, None, Trigger::Live) {
        Ok(saved) => lines.extend(saved),
        Err(refused) => lines.push(refused),
    }
    Ok(lines)
}
...
mod tests {
    ...
    fn going_live_publishes_the_workspace_and_saves_the_plane_at_once() {
        ...
        let said = switch(dir.path(), "alpha", true).expect("switched");
    ...
    fn going_local_stops_tracking_the_files_keeps_them_on_disk_and_saves() {
        ...
        let said = switch(dir.path(), "alpha", false).expect("local");
