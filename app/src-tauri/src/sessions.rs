...
use charter_core::session::{Session, Spec};
use std::sync::mpsc::RecvTimeoutError;

/// How many lines of history each session keeps.
pub const SCROLLBACK: usize = 5_000;

/// How often a view's thread looks up to see whether its pane has gone.
const AWAKE: Duration = Duration::from_millis(100);
...
/// A view being read by a thread of its own. Dropping this stops that thread, which closes
/// the view in the core.
struct Watcher {
...
impl Sessions {
    ...
    pub fn open(&self, opening: &Opening) -> Result<u32, String> {
        ...
        let engine = AlacrittyEngine::new(opening.size, SCROLLBACK);
    ...
    pub fn watch(&self, id: u32, mut sink: Sink) -> Result<Watching, String> {
        self.with(id, |running| {
            let view = running.session.attach();
            let size = view.size;
            let stopped = Arc::new(AtomicBool::new(false));
            // A thread of its own reads the view, so nothing the UI does runs on the
            // session's reading thread, and a slow pane holds up only itself.
                ...
                .spawn({
                    ...
                    move || {
                        ...
                        while !stopped.load(Ordering::Acquire) {
                            match view.output.recv_timeout(AWAKE) {
                                Ok(bytes) => sink(text.push(&bytes)),
                                // The session has ended, and its last output is delivered.
            ...
            running.views.insert(id, Watcher { stopped });
