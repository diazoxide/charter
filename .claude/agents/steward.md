...
- **You may delegate to these personas: `statusline`, `release`, `forge`.** Hand them a sub-task (Agent tool, `subagent_type: <name>`). Their tools are not auto-approved for you, and their vaults are not yours to open — do not name one to `charter secret …`. That second half is a rule you keep, not a wall charter holds: nothing refuses the vault name (`docs/personas.md` → Reusing another persona), which is exactly why it is written here. That is what `borrows:` is for, and it is deliberate: doing their work yourself should cost more than handing it over.
